...
st.title("\uc0\u55357 \u56693 \u65039 \u8205 \u9794 \u65039  10k-Word Forensic Style Diagnostician & Humanizer Canvas")\
...
if st.session_state.step == 1:\
    ...
    \
    if st.button("Initialize Deep Style Scan \uc0\u55357 \u56589 ", type="primary", use_container_width=True):\
            ...
            with st.spinner("Slicing text blocks and preparing tracking matrix variables..."):\
                ...
                for p in raw_paragraphs:\
                    st.session_state.document_chunks.append(\{\
                        "original": p,\
                        "current": p,\
                        "status": "\uc0\u55357 \u56628  Flagged (Pending Review)",\
...
# ----------------------------------------------------------------------\
    ...
    passed_p = sum(1 for c in st.session_state.document_chunks if c["status"] == "\uc0\u55357 \u57314  Humanized & Verified")\
    flagged_p = total_p - passed_p\
    \
    col_m1, col_m2, col_m3 = st.columns(3)\
    col_m1.metric("Total Paragraph Blocks", total_p)\
    col_m2.metric("\uc0\u55357 \u57314  Passed (Human Tone)", passed_p)\
    col_m3.metric("\uc0\u55357 \u56628  Flagged (AI Signature)", flagged_p)\
    \
    st.divider()\
    \
    ...
    if active_block["status"] == "\uc0\u55357 \u57314  Humanized & Verified":\
        st.success("\uc0\u55357 \u57314  Style Status: Verified Human Cadence. Excellent structural variance.")\
    else:\
        st.error(f"\uc0\u55357 \u56628  Style Status: \{active_block['status']\}")\
        \
    col_edit, col_diagnostics = st.columns([6, 5])\
    \
    with col_edit:\
        st.subheader("\uc0\u55357 \u56541  Live Paragraph Editor")\
        ...
        with col_buttons[0]:\
            if st.button("Execute Sentence-Level Style Audit \uc0\u9889 ", type="primary", use_container_width=True):\
                with st.spinner("Analyzing paragraph cadence profiles..."):\
                    ...
                    1. Highlight sentences that sound robotic due to monotone rhythm, predictable word choice, or clich\'e9 academic connectors.\
...
\
                    ...
                    try:\
                        ...
                        if audit_data["is_robotic"]:\
                            st.session_state.document_chunks[selected_index]["status"] = "\uc0\u55357 \u56628  Flagged (AI Signature Detected)"\
                        else:\
                            st.session_state.document_chunks[selected_index]["status"] = "\uc0\u55357 \u57314  Humanized & Verified"\
                            \
        ...
        with col_buttons[1]:\
            if active_block["blueprint"]:\
                if st.button("Apply Suggested Blueprint Instantly \uc0\u55357 \u56481 ", use_container_width=True):\
                    st.session_state.document_chunks[selected_index]["current"] = active_block["blueprint"]\
                    st.session_state.document_chunks[selected_index]["status"] = "\uc0\u55357 \u57314  Humanized & Verified"\
                    st.toast("Blueprint applied seamlessly!", icon="\uc0\u9989 ")\
                    st.rerun()\
\
    with col_diagnostics:\
        st.subheader("\uc0\u55357 \u56589  Forensic Metrics & Blueprints")\
        with st.container(border=True):\
            st.markdown("### \uc0\u55357 \u56522  Structural Vulnerability Analysis")\
            st.markdown(active_block["feedback"])\
            \
            if active_block["blueprint"]:\
                st.divider()\
                st.markdown("### \uc0\u55357 \u56481  Recommended Human Blueprint Rewrite")\
                st.info(active_block["blueprint"])\
\
    st.divider()\
    \
    # Export panel to stitch the full document back together cleanly\
    st.subheader("\uc0\u55357 \u56960  Reassemble Clean Final Manuscript")\
    if st.button("Compile & Render Full Humanized Text \uc0\u10145 \u65039 ", use_container_width=True):\
...
# ----------------------------------------------------------------------\
    ...
    with col_foot[0]:\
        if st.button("\uc0\u9198 \u65039  Return to Workbench"):\
            st.session_state.step = 2\
            st.rerun()\
    with col_foot[1]:\
        if st.button("Process an Entirely New Document Draft File \uc0\u55357 \u56529 ", use_container_width=True):\
            st.session_state.step = 1\
            st.session_state.document_chunks = []\
            st.rerun()}